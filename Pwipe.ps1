...
Function copy-Files-To-Ram-Drive() {
    if (-Not ((Test-Path "$($SYSTEM_DRIVE)\Pwipe.bat") -And (Test-Path "$($SYSTEM_DRIVE)\Pwipe.ps1"))) {
        Write-Host ""
        Write-Host "RAM Drive missing Pwipe files. Copying files necessary to be able to remove USB..." -ForegroundColor Green
        $JUNK | Out-File .\_._
        Copy-Item -Path .\_._ -Destination "$($SYSTEM_DRIVE)\_._"
        Copy-Item -Path .\Pwipe.bat -Destination "$($SYSTEM_DRIVE)\Pwipe.bat"
        Copy-Item -Path .\Pwipe.ps1 -Destination "$($SYSTEM_DRIVE)\Pwipe.ps1"
    }
    else {
        return #Files already exist, do exit function
    }
    Set-Location $ENV:SystemDrive
    Write-Host "Relaunching Pwipe from Ram Drive." -ForegroundColor Green
    Start-Sleep -Seconds 3
    Powershell.exe -File .\Pwipe.ps1
    Exit
}
